...
class Survival_plot():
	...
	def survival_plot(T1,E1,T2,E2,GT_input,primary_site,random_id,Low_Percentile,High_Percentile,survival_days,survival_select):
		...
		plot_path = f"{file_path}/../static/survival_data/plot_result/survival_plot_{primary_site.replace(' ','_').replace('(','').replace(')','')}_{GT_input}_{int(survival_days)}_{survival_select}.png"
		try:
			os.remove(plot_path)
		except OSError:
			pass
	...
	def survival_download(T1,E1,T2,E2,high_case,low_case,high_FPKM,low_FPKM,GT_input,primary_site,random_id,Low_Percentile,High_Percentile,survival_select):
		...
		with open(f"{file_path}/../static/survival_data//csv_result/{filename}", "w") as f:
			writer = csv.writer(f)
