...
def cal_pvalue_main(input_type, cancer, stage, high_percent, low_percent, input_pvalue):
    ...
    print(P_value_corr_FDR[1])
    df["FDR"] = P_value_corr_FDR[1]
    df["Bonferroni"] = P_value_corr_Bon[1]
    # df turn into dict of list
    df.drop(columns=["max_time"], inplace=True)
    df.to_csv(f"{current_path}/../static/survival/{primary_site}_{project}_{high_percent}_{low_percent}_{stage}_result.csv", index=False)
...
def survival_plot_realtime(project, primary_site, search_by, GT_input,random_id, Low_Percentile, High_Percentile, survival_select, survival_days:int =None):
    ...
    survival_data = survival_data_realtime(column_table,search_by,GT_input)
    print(survival_data)
    # survival_data = survival_data[1:]
    survival_str = ""
    case_id_list = []
    ...
    if (T2 != [] and E2 != []) and (T1 != [] and E1 != []):
        ...
        survival_data_to_csv(T1,E1,T2,E2,high_case,low_case,high_FPKM,low_FPKM,GT_input,primary_site,"",Low_Percentile,High_Percentile,survival_select, T3, E3, other_case, other_FPKM)
    else:
        p_value = 1
        survival_str = ' Survival analysis is not available for '+GT_input+' since more than half of the samples have zero expression.'
        img_str = ''
    return img_str, p_value

def survival_max_days(project: str, GT_input: str, search_by: str, survival_select: str) -> dict:
    table_name = '%s_%s_FPKM_Cufflinks'%(project,search_by)
    column_table = "%s|%s"%(survival_select,table_name)
    survival_data = "".join(survival_data_realtime(column_table,search_by,GT_input)).split(",")
...
def survival_data_realtime(column_table,search_by,GT_input):
    ...
    with sqlite3.connect(db_path, check_same_thread=False) as db_conn:
        ...
        if column_table.split('|')[0] != 'all stage':
            ...
            column=[]
            print("PRAGMA table_info(`{}`)".format(table_name))
...
def survival_data_to_csv(T1,E1,T2,E2,high_case,low_case,high_FPKM,low_FPKM,GT_input,primary_site,random_id,Low_Percentile,High_Percentile,survival_select, T3, E3, other_case, other_FPKM):
    output_data = [
                    ['Query: %s'%(GT_input)],
    ...
    for idx,row in enumerate(other_case):
        Status = 'Alive' if E3[idx] == False else 'Dead'
        output_data += [[row,T3[idx],Status,other_FPKM[idx],'']]
    ...
    with open(f"{current_path}/../static/survival/{filename}", "w") as f:
        writer = csv.writer(f)
